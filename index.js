...
;// ./src/application/pages/login.tsx

var login_defProp = Object.defineProperty;
var login_getOwnPropSymbols = Object.getOwnPropertySymbols;
var login_hasOwnProp = Object.prototype.hasOwnProperty;
var login_propIsEnum = Object.prototype.propertyIsEnumerable;
var login_defNormalProp = (obj, key, value) => key in obj ? login_defProp(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
var login_spreadValues = (a, b) => {
  for (var prop in b || (b = {}))
    if (login_hasOwnProp.call(b, prop))
      login_defNormalProp(a, prop, b[prop]);
  if (login_getOwnPropSymbols)
    for (var prop of login_getOwnPropSymbols(b)) {
      if (login_propIsEnum.call(b, prop))
        login_defNormalProp(a, prop, b[prop]);
    }
  return a;
};
...
const STORAGE_KEY = "rocketman_api_token";
const TG_AUTH_TTL = 5 * 60;
const loginState = {
  tab: "credentials",
  // credentials
  login: "",
  ...
  tgAuthKey: "",
  tgAuthSecondsLeft: 0,
  tgAuthShowCommand: false,
  tgAuthLoading: false,
  // loading the key
  tgAuthVerifying: false,
  // verifying after button click
  // shared
  loading: false,
  error: "",
  lang: "en"
...
const T = {
  ...
  tabApiKey: "login.tabApiKey",
  tabRegister: "login.tabRegister",
  tabTelegram: "login.tabTelegram",
  labelLogin: "login.labelLogin",
  labelPassword: "login.labelPassword",
  labelConfirmPassword: "login.labelConfirmPassword",
  labelApiKey: "login.labelApiKey",
  placeholderLogin: "login.placeholderLogin",
  placeholderPassword: "login.placeholderPassword",
  placeholderConfirmPassword: "login.placeholderConfirmPassword",
  placeholderApiKey: "login.placeholderApiKey",
  signIn: "login.signIn",
  signInApiKey: "login.signInApiKey",
  signUp: "login.signUp",
  signingIn: "login.signingIn",
  signingUp: "login.signingUp",
  orUseApiKey: "login.orUseApiKey",
  orUseCredentials: "login.orUseCredentials",
  switchToApiKey: "login.switchToApiKey",
  switchToCredentials: "login.switchToCredentials",
  errLoginRequired: "login.errLoginRequired",
  errPasswordRequired: "login.errPasswordRequired",
  errConfirmPasswordRequired: "login.errConfirmPasswordRequired",
  errPasswordMismatch: "login.errPasswordMismatch",
  errApiKeyRequired: "login.errApiKeyRequired",
  errRegistration: "login.errRegistration",
  errIncorrect: "login.errIncorrect",
  errInvalidKey: "login.errInvalidKey",
  errServer: "login.errServer",
  // telegram auth
  tgTitle: "login.tg.title",
  tgInstruction: "login.tg.instruction",
  tgCommandLabel: "login.tg.commandLabel",
  tgShowCommand: "login.tg.showCommand",
  tgHideCommand: "login.tg.hideCommand",
  tgCopyCommand: "login.tg.copyCommand",
  tgCopied: "login.tg.copied",
  tgTimerLabel: "login.tg.timerLabel",
  tgTimerExpired: "login.tg.timerExpired",
  tgVerifyButton: "login.tg.verifyButton",
  tgVerifying: "login.tg.verifying",
  tgRequestNew: "login.tg.requestNew",
  tgLoading: "login.tg.loading",
...
class LoginPage extends react.Component {
  constructor() {
    ...
    this.handleCredentialsSubmit = () => {
      ...
      if (!login.trim() || !password.trim()) {
        this.setState({ error: this.LangString(T.errLoginRequired) });
        return;
      }
      this.setState({ loading: true, error: "" });
      const referralHostId = readSavedReferralHostId();
      lib_axios.post(`/api/user/auth/web`, {
        login,
        password,
        ref: referralHostId || void 0
      }).then((res) => {
        var _a;
        const data = res.data;
        if (data.status && data.api_key) {
          if (referralHostId) clearSavedReferralHostId();
          this.saveAndSuccess(data.api_key);
        } else {
          this.setState({
            error: (_a = data.reason) != null ? _a : this.LangString(T.errIncorrect),
            loading: false
          });
        }
      }).catch(
        () => this.setState({ error: this.LangString(T.errServer), loading: false })
      );
    };
    // ─── Registration ─────────────────────────────────────────────────────────
    this.handleRegisterSubmit = () => {
      const { registerLogin, registerPassword, registerConfirmPassword, lang } = this.state;
      if (!registerLogin.trim()) {
        this.setState({ error: this.LangString(T.errLoginRequired) });
        return;
      }
      if (!registerPassword.trim()) {
        this.setState({ error: this.LangString(T.errPasswordRequired) });
        return;
      }
      if (!registerConfirmPassword.trim()) {
        this.setState({ error: this.LangString(T.errConfirmPasswordRequired) });
        return;
      }
      if (registerPassword !== registerConfirmPassword) {
        this.setState({ error: this.LangString(T.errPasswordMismatch) });
        return;
      }
      this.setState({ loading: true, error: "" });
      const referralHostId = readSavedReferralHostId();
      lib_axios.post(`/api/user/register`, {
        login: registerLogin,
        ...
        if (data.status && data.api_key) {
          ...
          this.setState({
            error: (_a = data.reason) != null ? _a : this.LangString(T.errRegistration),
            loading: false
          });
        }
      }).catch(
    ...
    this.handleApiKeySubmit = () => {
      ...
      if (!apiKey.trim()) {
        this.setState({ error: this.LangString(T.errApiKeyRequired) });
        return;
      }
      this.setState({ loading: true, error: "" });
      lib_axios.get(`/api/user/auth/token/${encodeURIComponent(apiKey)}`).then((res) => {
        var _a;
        const data = res.data;
        if (data.status) {
          this.saveAndSuccess(apiKey);
        } else {
          this.setState({
            error: (_a = data.reason) != null ? _a : this.LangString(T.errInvalidKey),
            loading: false
          });
        }
      }).catch(
    ...
    this.requestTgKey = () => {
      ...
      lib_axios.get("/api/user/auth/telegram/generate").then((res) => {
        const data = res.data;
        if (data.status && data.key) {
          ...
          this.setState({
            error: this.LangString(T.errServer),
            tgAuthLoading: false
          });
        }
      }).catch(
    ...
    this.handleTgVerify = () => {
      const { tgAuthKey } = this.state;
      if (!tgAuthKey) return;
      const referralHostId = readSavedReferralHostId();
      this.setState({ tgAuthVerifying: true, error: "" });
      const refQuery = referralHostId ? `?ref=${encodeURIComponent(referralHostId)}` : "";
      lib_axios.get(
        `/api/user/auth/telegram/verify/${encodeURIComponent(
          tgAuthKey
        )}${refQuery}`
      ).then((res) => {
        const data = res.data;
        if (data.status && data.api_key) {
          ...
          this.setState({
            tgAuthVerifying: false,
            error: this.LangString(T.tgErrNotSent)
          });
        }
      }).catch(
        () => this.setState({
  ...
  startTgTimer() {
    if (this.timerInterval) clearInterval(this.timerInterval);
    this.setState({ tgAuthSecondsLeft: TG_AUTH_TTL });
    this.timerInterval = setInterval(() => {
      this.setState((prev) => {
        const next = prev.tgAuthSecondsLeft - 1;
        if (next <= 0) {
          clearInterval(this.timerInterval);
          this.timerInterval = null;
          return { tgAuthSecondsLeft: 0 };
        }
        return { tgAuthSecondsLeft: next };
      });
    }, 1e3);
  }
  // ─── Shared ──────────────────────────────────────────────────────────────
  saveAndSuccess(apiKey) {
    telegram_WebApp.api_token = apiKey;
  ...
  renderTgTab() {
    ...
    const botName = Core.GetConfigValue("projectBot");
    const command = `/auth ${tgAuthKey}`;
    const maskedCommand = `/auth ${"\u2022".repeat(Math.max(tgAuthKey.length, 8))}`;
    const timerExpired = tgAuthKey && tgAuthSecondsLeft === 0;
    const showRequestNew = !!error || !!timerExpired;
    if (tgAuthLoading) {
      return /* @__PURE__ */ react.createElement("div", { className: "login-form" }, /* @__PURE__ */ react.createElement("div", { className: "login-tg-loading" }, this.LangString(T.tgLoading)));
    }
    return /* @__PURE__ */ react.createElement("div", { className: "login-form" }, /* @__PURE__ */ react.createElement(
      "div",
      {
        className: "login-tg-instruction",
        dangerouslySetInnerHTML: {
          __html: this.LangString(T.tgInstruction, `${botName}`)
        }
      }
    ), tgAuthKey && /* @__PURE__ */ react.createElement(react.Fragment, null, /* @__PURE__ */ react.createElement("div", { className: "login-tg-command-block" }, /* @__PURE__ */ react.createElement("span", { className: "login-tg-command-label" }, this.LangString(T.tgCommandLabel)), /* @__PURE__ */ react.createElement("div", { className: "login-tg-command-row" }, /* @__PURE__ */ react.createElement("code", { className: "login-tg-command" }, tgAuthShowCommand ? command : maskedCommand), /* @__PURE__ */ react.createElement("div", { className: "login-tg-command-btns" }, /* @__PURE__ */ react.createElement(
      "button",
      {
        className: "small",
        onClick: () => this.setState({ tgAuthShowCommand: !tgAuthShowCommand })
      },
      this.LangString(
        tgAuthShowCommand ? T.tgHideCommand : T.tgShowCommand
      )
      ...
      "button",
      {
        ...
        onClick: () => {
          ...
          telegram_WebApp.showAlert(this.LangString(T.tgCopied));
        }
      },
      this.LangString(T.tgCopyCommand)
    )))), /* @__PURE__ */ react.createElement("div", { className: `login-tg-timer${timerExpired ? " expired" : ""}` }, timerExpired ? this.LangString(T.tgTimerExpired) : this.LangString(
      T.tgTimerLabel,
      this.formatTimer(tgAuthSecondsLeft)
    ))), error && /* @__PURE__ */ react.createElement("div", { className: "login-error" }, error), showRequestNew && /* @__PURE__ */ react.createElement("button", { className: "login-alt", onClick: this.requestTgKey }, this.LangString(T.tgRequestNew)), tgAuthKey && !timerExpired && !showRequestNew && /* @__PURE__ */ react.createElement(
      "button",
      {
        className: "login-submit",
        disabled: tgAuthVerifying,
        onClick: this.handleTgVerify
      },
      tgAuthVerifying ? this.LangString(T.tgVerifying) : this.LangString(T.tgVerifyButton)
    ), this.renderBottom());
  }
  // ─── Render ───────────────────────────────────────────────────────────────
  renderBottom() {
    const botName = Core.GetConfigValue("projectBot");
    return /* @__PURE__ */ react.createElement(react.Fragment, null, /* @__PURE__ */ react.createElement("div", { className: "login-divider" }), /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement(
      "select",
      {
        onChange: (e) => {
          e.preventDefault();
    ...
    )), botName && /* @__PURE__ */ react.createElement(
      "button",
      {
        className: "login-submit",
        onClick: () => {
  ...
  render() {
    ...
    return /* @__PURE__ */ react.createElement("div", { className: "login-page" }, /* @__PURE__ */ react.createElement("div", { className: "login-card" }, /* @__PURE__ */ react.createElement("div", { className: "login-header" }, /* @__PURE__ */ react.createElement("span", { className: "rotate" }, "\u{1F680}"), /* @__PURE__ */ react.createElement("h1", null, Core.GetConfigValue("projectName")), /* @__PURE__ */ react.createElement("span", { className: "rotate" }, "\u{1F680}")), /* @__PURE__ */ react.createElement("div", { className: "login-tabs" }, /* @__PURE__ */ react.createElement(
      "button",
      {
        ...
        onClick: () => this.switchTab("credentials")
      },
      this.LangString(T.tabCredentials)
    ), /* @__PURE__ */ react.createElement(
      "button",
      {
      ...
      this.LangString(T.tabRegister)
    ), /* @__PURE__ */ react.createElement(
      "button",
      {
        className: tab === "apikey" ? "active" : "",
        onClick: () => this.switchTab("apikey")
      },
      this.LangString(T.tabApiKey)
    ), /* @__PURE__ */ react.createElement(
      "button",
      {
        className: tab === "telegram" ? "active" : "",
        onClick: () => this.switchTab("telegram")
      },
      this.LangString(T.tabTelegram)
    )), tab === "credentials" && /* @__PURE__ */ react.createElement(react.Fragment, null, error && /* @__PURE__ */ react.createElement("div", { className: "login-error" }, error), /* @__PURE__ */ react.createElement("div", { className: "login-form" }, /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement("label", null, this.LangString(T.labelLogin)), /* @__PURE__ */ react.createElement(
      "input",
      {
        type: "text",
        placeholder: this.LangString(T.placeholderLogin),
        value: login,
        onChange: (e) => this.setState({ login: e.target.value }),
        onKeyDown: (e) => e.key === "Enter" && this.handleCredentialsSubmit()
      }
    )), /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement("label", null, this.LangString(T.labelPassword)), /* @__PURE__ */ react.createElement(
      "input",
      {
        type: "password",
        placeholder: this.LangString(T.placeholderPassword),
        value: password,
      ...
      loading ? this.LangString(T.signingIn) : this.LangString(T.signIn)
    ), this.renderBottom())), tab === "register" && /* @__PURE__ */ react.createElement(react.Fragment, null, error && /* @__PURE__ */ react.createElement("div", { className: "login-error" }, error), /* @__PURE__ */ react.createElement("div", { className: "login-form" }, /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement("label", null, this.LangString(T.labelLogin)), /* @__PURE__ */ react.createElement(
      "input",
      {
        type: "text",
        placeholder: this.LangString(T.placeholderLogin),
        value: this.state.registerLogin,
        onChange: (e) => this.setState({ registerLogin: e.target.value })
      }
    )), /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement("label", null, this.LangString(T.labelPassword)), /* @__PURE__ */ react.createElement(
      "input",
      {
        ...
        onKeyDown: (e) => e.key === "Enter" && this.handleRegisterSubmit()
      }
    )), /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement("label", null, this.LangString(T.labelConfirmPassword)), /* @__PURE__ */ react.createElement(
      "input",
      {
        type: "password",
        placeholder: this.LangString(T.placeholderConfirmPassword),
      ...
      loading ? this.LangString(T.signingUp) : this.LangString(T.signUp)
    ), this.renderBottom())), tab === "apikey" && /* @__PURE__ */ react.createElement(react.Fragment, null, error && /* @__PURE__ */ react.createElement("div", { className: "login-error" }, error), /* @__PURE__ */ react.createElement("div", { className: "login-form" }, /* @__PURE__ */ react.createElement("div", { className: "login-field" }, /* @__PURE__ */ react.createElement("label", null, this.LangString(T.labelApiKey)), /* @__PURE__ */ react.createElement(
      "input",
      {
        type: "text",
        placeholder: this.LangString(T.placeholderApiKey),
        value: apiKey,
      ...
      loading ? this.LangString(T.signingIn) : this.LangString(T.signInApiKey)
    ), this.renderBottom())), tab === "telegram" && this.renderTgTab()));
  }
}
function restoreApiToken() {
  const token = localStorage.getItem(STORAGE_KEY);
  if (token) WebApp.api_token = token;
  return token;
}

;// ./src/application/modules/telegram.ts
...
class Btn {
  constructor() {
    this.onClickList = /* @__PURE__ */ new Set();
    this.text = "";
    this.color = "";
    this.textColor = "";
    ...
    this.onClick = (callback) => {
      if (!this.onClickList.has(callback)) this.onClickList.add(callback);
      return this;
    };
    this.offClick = (callback) => {
      this.onClickList.delete(callback);
      return this;
    };
    this.show = () => {
      this.isVisible = true;
    ...
    this.showProgress = (leaveActive) => {
      this.isProgressVisible = true;
      if (!leaveActive && this.isActive) {
        this.isActive = false;
      }
      return this;
    };
    ...
    this.setParams = (params) => {
      console.log(`set button params`, params);
      if (typeof params.text === "string") this.text = params.text;
      if (typeof params.color === "string") this.color = params.color;
      if (typeof params.text_color === "string")
        this.textColor = params.text_color;
      if (typeof params.is_active === "boolean") this.isActive = params.is_active;
      if (typeof params.is_visible === "boolean")
        this.isVisible = params.is_visible;
      return this;
    };
  }
}
const initButtonContructor = () => {
  return new Btn();
};
class BtnSimple {
  constructor() {
  ...
  onClick(callback) {
    if (!this.onClickList.has(callback)) {
      this.onClickList.add(callback);
    }
    return this;
  }
  offClick(callback) {
...
const initSimpleButtonContructor = () => {
  return new BtnSimple();
};
const telegram_WebApp = new class {
  constructor() {
    this.BackButton = initSimpleButtonContructor();
    this.MainButton = initButtonContructor();
    this.SecondaryButton = initButtonContructor();
    this.SettingsButton = initSimpleButtonContructor();
    this.isClosingConfirmationEnabled = false;
    if (isTelegramEnv()) {
      this.BackButton = window.Telegram.WebApp.BackButton;
      this.MainButton = window.Telegram.WebApp.MainButton;
      this.SecondaryButton = window.Telegram.WebApp.SecondaryButton;
      this.SettingsButton = window.Telegram.WebApp.SettingsButton;
      window.Telegram.WebApp.disableVerticalSwipes();
      window.Telegram.WebApp.expand();
      this.isClosingConfirmationEnabled = window.Telegram.WebApp.isClosingConfirmationEnabled;
      this.initData = Telegram.WebApp.initData;
      setInterval(() => {
        if (window.Telegram.WebApp.isClosingConfirmationEnabled !== this.isClosingConfirmationEnabled)
          window.Telegram.WebApp.isClosingConfirmationEnabled = this.isClosingConfirmationEnabled;
      }, 100);
    }
  }
  get api_token() {
  ...
  showConfirm(text, cb) {
    if (isTelegramEnv()) {
      return window.Telegram.WebApp.showConfirm(text, cb);
    } else {
  ...
  openInvoice(url, cb) {
    if (isTelegramEnv()) {
      return window.Telegram.WebApp.openInvoice(url, cb);
    } else {
      requestAnimationFrame(() => {
  ...
  showAlert(text, cb) {
    requestAnimationFrame(() => {
      if (isTelegramEnv()) window.Telegram.WebApp.showAlert(text, cb);
      else {
        alert(text);
        if (cb) cb();
      }
...
window.WebApp = telegram_WebApp;

;// ./src/shared/sse.ts



function sse_isTelegramEnv() {
  var _a;
  if (!((_a = window.Telegram) == null ? void 0 : _a.WebApp)) return false;
  const platform = window.Telegram.WebApp.platform;
  return !!platform && platform !== "unknown";
