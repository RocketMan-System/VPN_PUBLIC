...
.payment-crypto-section .payment-crypto-form h2 {
  text-align: center;
  color: var(--tg-theme-accent-text-color, #8774e1);
  margin-bottom: 16px;
}
...
.payment-crypto-section .payment-crypto-form .crypto-details .crypto-row .crypto-label {
  color: var(--tg-theme-subtitle-text-color, #aaaaaa);
}
...
.payment-crypto-section .payment-crypto-form .crypto-error {
  color: var(--tg-theme-destructive-text-color, #ff5842);
  text-align: center;
  font-weight: bold;
  margin: 16px 0;
}
...
class SseClass {
  ...
  Init() {
    ...
    this.source = new EventSource(
      `/tgapi/events?verify=${System.toBinary(Telegram.WebApp.initData)}`
    );
